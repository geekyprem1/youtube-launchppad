...
export async function processRecommendations(request: RecommendationRequest): Promise<APIResponse> {
  ...
  const rawIdeas = await generateRawIdeas(request.niche);

  // 2. Feature Extraction
  const features = extractFeatures(rawData);
  let ruleVersion = "1.0";

  // 3. Process ideas SEQUENTIALLY to avoid overloading OpenRouter
  const formattedIdeas: any[] = [];
  ...
  for (let i = 0; i < rawIdeas.length; i++) {
    ...
    // AI Reasoning - graceful fallback per idea if AI fails
    ...
    try {
      ...
      const aiRawResponse = await generateAIResponse(
        ...
        { json: true, promptVersion: "recommendation.v1", max_tokens: 500 }
      );
      analysis = validateAIResponse(aiRawResponse, IdeaAnalysisSchema, FALLBACK_ANALYSIS);
    } catch (aiErr) {
      console.error(`[Recommendations] AI analysis failed for "${rawIdea.topic}", using fallback:`, aiErr);
    }

    // Map UI color formatting for breakdowns
    const breakdownUI = Object.entries(opportunity.breakdown).map(([_k, v]) => ({
      label: v.label,
      value: `+${v.value}`,
    ...
    const reachText = reachMax > 1000
      ? `${Math.round(reachMin / 1000)}K - ${Math.round(reachMax / 1000)}K`
      : `${reachMin} - ${reachMax}`;
